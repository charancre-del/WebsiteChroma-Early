 ...
 * @since 1.0.0
 * 
 * @deprecated 2.0.0 This file's schema output is SUPERSEDED by Early Start SEO Pro plugin.
 ...
 */
if (!function_exists('earlystart_general_content_schema')) {
function earlystart_general_content_schema() {
...
// add_action('wp_head', 'earlystart_general_content_schema', 1);

/**
 * Add Organization Schema to Homepage
 */
if (!function_exists('earlystart_organization_schema')) {
function earlystart_organization_schema()
{
        if (!is_front_page()) {
                return;
        }
        ...
        $logo = get_post_meta($homepage_id, 'schema_org_logo', true) ?: earlystart_get_global_setting('global_logo', '');
        $description = get_post_meta($homepage_id, 'schema_org_description', true) ?: earlystart_global_seo_default_description();
        ...
        $phonetic = get_theme_mod('earlystart_global_brand_phonetic', '');
        if ($phonetic) {
...
add_action('send_headers', 'earlystart_seo_headers');

/**
 * Add WebSite Schema to Homepage (for Sitelinks Search Box)
 */
if (!function_exists('earlystart_website_schema')) {
function earlystart_website_schema()
{
        if (!is_front_page()) {
                return;
        }
...
// DISABLED: Schema now handled by Early Start SEO Pro plugin
// add_action('wp_head', 'earlystart_website_schema');

/**
 * Add LocalBusiness Schema to Location Pages
 */
if (!function_exists('earlystart_location_schema')) {
function earlystart_location_schema()
{
        ...
        $description = get_post_meta($location_id, 'schema_loc_description', true) ?: (get_the_excerpt() ?: earlystart_trimmed_excerpt(30, $location_id));
...
// DISABLED: Schema now handled by Early Start SEO Pro plugin
// add_action('wp_head', 'earlystart_location_schema');

/**
 * Add Service Schema to City Pages
 */
if (!function_exists('earlystart_city_schema')) {
function earlystart_city_schema()
{
        if (!is_singular('city')) {
                return;
        }
        ...
        $schema = array(
                ...
                'description' => get_the_excerpt() ?: "Evidence-based pediatric therapy services in $city_name, GA, including autism diagnosis, ABA therapy, behavioral health, speech therapy, and occupational therapy.",
                'url' => get_permalink()
        );
...
// DISABLED: Schema now handled by Early Start SEO Pro plugin
// add_action('wp_head', 'earlystart_city_schema');

/**
 * Add Service Schema to Program Pages
 */
if (!function_exists('earlystart_program_schema')) {
function earlystart_program_schema()
{
...
// DISABLED: Schema now handled by Early Start SEO Pro plugin
// add_action('wp_head', 'earlystart_program_schema');

/**
 * Add FAQPage Schema to Homepage (when FAQ section exists)
 */
if (!function_exists('earlystart_faq_schema')) {
function earlystart_faq_schema()
{
    return;
}
}
// add_action('wp_head', 'earlystart_faq_schema');
 ...
 */
function earlystart_truncate_social_title($title, $max_length = 60)
{
 ...
 */
function earlystart_get_social_image()
{
        $image_url = '';
        ...
        if (empty($image_url)) {
                $post_id = get_the_ID();
                if ($post_id) {
                        $custom_image = get_post_meta($post_id, '_earlystart_social_image', true);
        ...
        if (empty($image_url)) {
                $hero_image = get_theme_mod('earlystart_home_hero_image');
                if (!empty($hero_image)) {
 ...
 * Open Graph Tags
 */
function earlystart_og_tags()
{
        // Truncate OG title to 60 characters max
        $og_title = earlystart_truncate_social_title(get_the_title(), 60);
        ...
        $social_image = earlystart_get_social_image();
        ...
        $description = get_the_excerpt() ?: earlystart_global_seo_default_description();
        echo '<meta property="og:description" content="' . esc_attr(wp_strip_all_tags($description)) . '" />' . "\n";
}
add_action('wp_head', 'earlystart_og_tags', 5);

/**
 * Twitter Card Tags
 */
function earlystart_twitter_cards()
{
        // Truncate Twitter title to 55 characters max
        $twitter_title = earlystart_truncate_social_title(get_the_title(), 55);
        ...
        $twitter_handle = get_option('earlystart_twitter_handle', '');
        if ($twitter_handle) {
                echo '<meta name="twitter:site" content="' . esc_attr($twitter_handle) . '" />' . "\n";
        }
}
add_action('wp_head', 'earlystart_twitter_cards', 6);

/**
 * Hreflang Tags for EN/ES
 */
function earlystart_hreflang_tags()
{
        $post_id = get_the_ID();
...
if (!class_exists('earlystart_Multilingual_Manager')) {
    add_action('wp_head', 'earlystart_hreflang_tags', 1);
}
 ...
 * Shared meta description output with fallbacks
 */
function earlystart_shared_meta_description()
{
        ...
        if (is_singular('location')) {
                ...
                $description = earlystart_global_seo_default_description();
                if (empty($description)) {
                        $description = get_bloginfo('description');
                }
                if (empty($description)) {
                        $description = get_bloginfo('name') . ' provides pediatric therapy services across Metro Atlanta, including autism diagnosis, ABA therapy, behavioral health, speech therapy, and occupational therapy.';
        ...
        if (empty($description)) {
                ...
                if (empty($description) && $post_id) {
                        $description = has_excerpt($post_id) ? get_the_excerpt($post_id) : earlystart_trimmed_excerpt(32, $post_id);
                }
 ...
 */
function earlystart_meta_keywords()
{
        // 1. Manual Override
        $post_id = get_the_ID();
...
add_action('wp_head', 'earlystart_meta_keywords', 3);

/**
 * Custom Sitemap.xml
 */
function earlystart_custom_sitemap()
{
...
// add_action('template_redirect', 'earlystart_custom_sitemap');
 ...
 */
function earlystart_register_sitemap_rewrites()
{
        add_rewrite_rule('^sitemap\.xml$', 'index.php?sitemap=xml', 'top');
}
// Disable Custom Sitemap Rewrite Rules in favor of Native
// add_action('init', 'earlystart_register_sitemap_rewrites');

/**
 * Register Sitemap Query Var
 */
function earlystart_register_sitemap_query_var($vars)
{
        $vars[] = 'sitemap';
        return $vars;
}
add_filter('query_vars', 'earlystart_register_sitemap_query_var');
 ...
 */
function earlystart_sitemap_config()
{
...
add_action('init', 'earlystart_sitemap_config');

/**
 * Custom Robots.txt
 */
function earlystart_custom_robots_txt($output)
{
...
function earlystart_city_faq_schema_output()
{
        ...
        $a1 = "Yes. Families in $city can access pediatric therapy through nearby Chroma Early Start clinics, including autism diagnosis, ABA therapy, behavioral health, speech therapy, and occupational therapy based on clinical fit.";
