...
class earlystart_Menu_Sync {
    ...
    public static function register_admin_page() {
        add_management_page(
            __('Early Start Menu Sync', 'earlystart-early-learning'),
            __('Early Start Menu Sync', 'earlystart-early-learning'),
            'manage_options',
            'chroma-menu-sync',
    ...
    public static function render_admin_page() {
        ?>
        <div class="wrap">
            <h1><?php _e('Early Start Menu Sync Utility', 'earlystart-early-learning'); ?></h1>
            <p><?php _e('Use this tool to duplicate your existing English menus into Spanish menu locations. This allows you to have a starting point for translating menu items.', 'earlystart-early-learning'); ?></p>
            
            <div class="card" style="max-width: 600px; padding: 20px; margin-top: 20px;">
                <h2><?php _e('Sync Menus', 'earlystart-early-learning'); ?></h2>
                <p><?php _e('This will performed the following actions:', 'earlystart-early-learning'); ?></p>
                <ul style="list-style-type: disc; margin-left: 20px;">
                    <li><?php _e('Look for menus assigned to "Primary", "Footer", and "Footer Contact" locations.', 'earlystart-early-learning'); ?></li>
                    <li><?php _e('Create new menus with " (ES)" suffix if they don\'t exist.', 'earlystart-early-learning'); ?></li>
                    <li><?php _e('Copy all menu items from the English menu to the Spanish menu.', 'earlystart-early-learning'); ?></li>
                    <li><?php _e('Assign the new menus to the corresponding Spanish theme locations.', 'earlystart-early-learning'); ?></li>
                </ul>
                ...
                <form method="post" action="">
                    ...
                    <p>
                        <label>
                            ...
                            <strong><?php _e('Force Overwrite:', 'earlystart-early-learning'); ?></strong> <?php _e('If checked, existing items in Spanish menus will be deleted and replaced with English items.', 'earlystart-early-learning'); ?>
                        </label>
                    </p>
                    <p class="submit">
                        <input type="submit" name="earlystart_sync_menus" id="submit" class="button button-primary" value="<?php esc_attr_e('Sync Menus', 'earlystart-early-learning'); ?>">
                    </p>
                </form>
            </div>
        </div>
    ...
    public static function process_sync() {
        ...
        foreach ($mappings as $en_loc => $es_loc) {
            ...
            if (!isset($locations[$en_loc]) || $locations[$en_loc] == 0) {
                $messages[] = sprintf(__('Skipped "%s": No menu assigned to English location.', 'earlystart-early-learning'), $en_loc);
                continue;
            }
            ...
            if (!$es_menu_obj) {
                ...
                if (is_wp_error($es_menu_id)) {
                    $messages[] = sprintf(__('Error creating menu "%s": %s', 'earlystart-early-learning'), $es_menu_name, $es_menu_id->get_error_message());
                    continue;
                }
                $es_menu_obj = wp_get_nav_menu_object($es_menu_id);
                $messages[] = sprintf(__('Created new menu: "%s"', 'earlystart-early-learning'), $es_menu_name);
            } else {
                $es_menu_id = $es_menu_obj->term_id;
                $messages[] = sprintf(__('Found existing menu: "%s"', 'earlystart-early-learning'), $es_menu_name);
            }
            ...
            if (!empty($es_items) && !$force_overwrite) {
                $messages[] = sprintf(__('Skipped syncing items for "%s": Menu is not empty and overwrite is disabled.', 'earlystart-early-learning'), $es_menu_name);
            } else {
                ...
                if ($en_items) {
                    ...
                    $messages[] = sprintf(__('Synced %d items to "%s".', 'earlystart-early-learning'), count($en_items), $es_menu_name);
                }
            }
