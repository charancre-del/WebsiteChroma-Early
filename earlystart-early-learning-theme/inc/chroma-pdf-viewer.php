<?php
/**
 * Early Start PDF Shortcode & Viewer
 * 
...
function earlystart_pdf_shortcode($atts)
{
    ...
    <div
        ...
        <div class="h-48 relative overflow-hidden">
            ...
            <!-- Read Badge -->
            <div
                class="absolute bottom-3 right-3 z-10 bg-white/90 backdrop-blur px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider text-brand-ink shadow-sm opacity-0 group-hover:opacity-100 transform translate-y-2 group-hover:translate-y-0 transition-all duration-300">
                <i class="fa-solid fa-book-open mr-1 text-<?php echo esc_attr($args['color']); ?>"></i>
        ...
        <div class="p-5">
            <h3
                class="font-bold text-lg text-brand-ink leading-tight mb-1 group-hover:text-<?php echo esc_attr($args['color']); ?> transition-colors">
...
function earlystart_render_pdf_modal()
{
    ...
    <div id="chroma-pdf-modal" class="fixed inset-0 hidden" role="dialog" aria-modal="true">
        ...
        <div class="absolute inset-0 md:inset-6 flex flex-col pointer-events-none p-4 md:p-10">
            ...
            <!-- Branded Pro Toolbar -->
            <div id="chroma-pdf-toolbar"
                class="bg-brand-ink text-white rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.3)] mx-auto mb-6 px-4 md:px-8 py-4 flex items-center justify-between gap-4 md:gap-10 pointer-events-auto border border-white/10 animate-fade-in-down w-full max-w-4xl">
                <!-- Branding & Title -->
                <div class="flex items-center gap-4 border-r border-white/10 pr-6 mr-2">
                    ...
                    <div class="hidden md:block">
                        <span class="text-[10px] uppercase tracking-[0.2em] font-bold text-white/50 block mb-0.5">Pro
                            Viewer</span>
                        <h3 class="font-serif font-bold text-sm text-white truncate max-w-[200px]" id="chroma-pdf-title">
                            Document</h3>
                    </div>
                </div>
                ...
                <div class="flex items-center gap-2 md:gap-4 bg-white/5 rounded-full px-4 py-1.5 border border-white/5">
                    <button id="chroma-pdf-prev"
                        class="w-8 h-8 rounded-full flex items-center justify-center transition-all disabled:opacity-10 disabled:cursor-not-allowed"
                        title="Previous Page">
                        <i class="fa-solid fa-arrow-left"></i>
                    </button>
                    <span class="text-xs md:text-sm font-mono tracking-[0.2em] text-white/90 px-2 min-w-[60px] text-center">
                        <span id="chroma-pdf-page-num" class="font-bold text-white">1</span> <span
                            class="mx-1 opacity-40">/</span> <span id="chroma-pdf-page-count">--</span>
                    </span>
                    <button id="chroma-pdf-next"
                        class="w-8 h-8 rounded-full flex items-center justify-center transition-all disabled:opacity-10 disabled:cursor-not-allowed"
                        title="Next Page">
                ...
                <div class="flex items-center gap-2 border-l border-white/10 pl-6 ml-2">
                    ...
                    <button id="chroma-pdf-close"
                        class="w-10 h-10 rounded-full hover:bg-chroma-red flex items-center justify-center transition-all text-white/80 hover:text-white"
                        title="Close Content">
            ...
            <div class="flex-grow relative flex items-start justify-center overflow-hidden pointer-events-auto"
                id="chroma-pdf-canvas-container">
                <!-- Loader Widget -->
                <div id="chroma-pdf-loader"
                    class="absolute inset-0 z-[10] flex flex-col items-center justify-center text-white bg-brand-ink/40 backdrop-blur-sm rounded-3xl">
                    <div class="w-14 h-14 border-4 border-white/10 border-t-chroma-red rounded-full animate-spin mb-6">
                    </div>
                ...
                <!-- PDF Render Paper -->
                <div
                    class="max-w-full max-h-full overflow-auto custom-scrollbar rounded-xl shadow-[0_30px_60px_-15px_rgba(0,0,0,0.5)] bg-slate-100 p-1 md:p-4">
    ...
    <style>
        ...
        #chroma-pdf-prev:hover,
        #chroma-pdf-next:hover {
            background: #1DABCC !important;
            /* chroma-blue */
            border-color: #1DABCC;
            transform: scale(1.15);
        }

        #chroma-pdf-prev:disabled,
        #chroma-pdf-next:disabled {
