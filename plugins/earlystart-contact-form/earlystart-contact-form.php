...
{
    register_setting('earlystart_contact_options', 'earlystart_contact_fields', array(
        'type' => 'string',
        'sanitize_callback' => 'earlystart_contact_sanitize_json',
        'default' => wp_json_encode(earlystart_contact_default_fields())
    ));

    register_setting('earlystart_contact_options', 'earlystart_contact_webhook_url', array(
        'type' => 'string',
        'sanitize_callback' => 'esc_url_raw',
        'default' => ''
    ));

    register_setting('earlystart_contact_options', 'earlystart_contact_email_recipient', array(
        'type' => 'string',
    ...
    // GHL Embed Settings
    register_setting('earlystart_contact_options', 'earlystart_contact_form_id', array(
        'type' => 'string',
        'default' => 'ibinKhrBmF0n4S5tFcz6',
        'sanitize_callback' => 'sanitize_text_field'
    ));
    register_setting('earlystart_contact_options', 'earlystart_contact_form_height', array(
        'type' => 'integer',
        'default' => 779,
        'sanitize_callback' => 'absint'
    ));
    register_setting('earlystart_contact_options', 'earlystart_contact_form_name', array(
        'type' => 'string',
        'default' => 'Contact Us- Chroma Early Start',
        'sanitize_callback' => 'sanitize_text_field'
    ));
    register_setting('earlystart_contact_options', 'earlystart_contact_lazy_load', array(
        'type' => 'boolean',
        'default' => true,
        'sanitize_callback' => 'rest_sanitize_boolean'
    ));
    register_setting('earlystart_contact_options', 'earlystart_contact_lazy_delay', array(
        'type' => 'integer',
        'default' => 2000,
        'sanitize_callback' => 'absint'
    ));
...
function earlystart_contact_sanitize_json($input)
{
    ...
    if (json_last_error() !== JSON_ERROR_NONE) {
        add_settings_error('earlystart_contact_fields', 'invalid_json', 'Invalid JSON format. Changes not saved.');
...
function earlystart_contact_admin_menu()
{
    add_options_page(
        ...
        'Contact Form',
        'manage_options',
        'earlystart-contact-form',
        'earlystart_contact_settings_page_html'
    );
}
...
function earlystart_contact_settings_page_html()
{
    ...
    <div class="wrap">
        ...
        <form action="options.php" method="post" id="earlystart-contact-settings-form">
            <?php
            settings_fields('earlystart_contact_options');
            do_settings_sections('earlystart_contact_options');

            $fields_json = get_option('earlystart_contact_fields', wp_json_encode(earlystart_contact_default_fields()));
            $webhook_url = get_option('earlystart_contact_webhook_url', '');
            ...
            <table class="form-table">
                <tr valign="top">
                    ...
                    <td>
                        <input type="email" name="earlystart_contact_email_recipient"
                ...
                <tr valign="top">
                    ...
                    <td>
                        <input type="url" name="earlystart_contact_webhook_url"
                ...
                <tr valign="top">
                    ...
                    <td>
                        ...
                        <input type="hidden" name="earlystart_contact_fields" id="earlystart_contact_fields_input"
    ...
    <style>
        ...
        .earlystart-input-group textarea {
            width: 100%;
        }
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const input = document.getElementById('earlystart_contact_fields_input');
...
function earlystart_contact_form_shortcode()
{
    ...
    $form_name = get_option('earlystart_contact_form_name', 'Contact Us- Chroma Early Start');
    ...
    <style>
        .earlystart-contact-form-wrapper {
            width: 100%;
            margin: 0 auto;
        }

        .earlystart-contact-form-wrapper .earlystart-ghl-iframe-container {
            position: relative;
            overflow: hidden;
            border-radius: 0.75rem;
        }

        .earlystart-contact-form-wrapper .earlystart-ghl-iframe-container iframe {
            display: block;
        }
    </style>
 ...
 */
function earlystart_handle_contact_submission()
{
    if (!isset($_POST['earlystart_contact_submit']) || !wp_verify_nonce(wp_unslash($_POST['earlystart_contact_nonce'] ?? ''), 'earlystart_contact_submit')) {
        return;
    }
    ...
    if (!is_array($fields)) {
        $fields = earlystart_contact_default_fields();
    }
    ...
    $redirect_target = !empty($_POST['earlystart_contact_redirect']) ? esc_url_raw(wp_unslash($_POST['earlystart_contact_redirect'])) : (wp_get_referer() ?: $redirect_fallback);
    ...
    $global_recipients_str = get_option('earlystart_contact_email_recipient', get_option('admin_email'));
...
add_action('template_redirect', 'earlystart_handle_contact_submission');
