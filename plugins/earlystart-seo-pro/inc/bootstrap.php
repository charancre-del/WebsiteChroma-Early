...
// Load Theme Schema Compatibility
...
$meta_boxes = [
    ...
    'class-general-llm-context.php',
    'class-general-llm-prompt.php',
    ...
    'class-location-advanced-schema.php',
    'class-spanish-content.php'
];

foreach ($meta_boxes as $file) {
    earlystart_safe_require(__DIR__ . '/meta-boxes/' . $file);
}
...
if (!function_exists('earlystart_advanced_seo_init')) {
    function earlystart_advanced_seo_init()
    {
        ...
        $meta_classes = [
            ...
            'earlystart_General_LLM_Context',
            'earlystart_General_LLM_Prompt',
        ...
        // Modular Schemas from Schema Builder
        if (class_exists('earlystart_Schema_Injector'))
            add_action('wp_head', ['earlystart_Schema_Injector', 'output_modular_schemas'], 20);

        // Flush Rewrite Rules if KML rule is missing
        if (get_option('earlystart_seo_flush_rewrite_v1') !== 'done') {
            flush_rewrite_rules();
            update_option('earlystart_seo_flush_rewrite_v1', 'done');
        }
    }
...
if (!function_exists('earlystart_seo_missing_files_notice')) {
    function earlystart_seo_missing_files_notice()
    {
        ...
        if (!empty($earlystart_missing_seo_files) && current_user_can('manage_options')) {
            ...
            echo '<p><strong>Early Start SEO Module Warning:</strong> The following files are missing and could not be loaded:</p>';
            echo '<ul>';
            foreach ($earlystart_missing_seo_files as $file) {
                echo '<li>' . esc_html($file) . '</li>';
            }
            echo '</ul>';
            echo '<p>Please ensure all files are uploaded to <code>plugins/earlystart-seo-pro/inc/</code>.</p>';
            echo '</div>';
        }
    }
