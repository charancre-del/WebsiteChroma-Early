...
class earlystart_Author_Tags
{
    ...
    public function output_author_meta() {
        if (!get_option('earlystart_seo_show_author_meta', true)) {
            return;
        }
        ...
        if ($author) {
            ...
            earlystart_Schema_Registry::register($schema, ['source' => 'author-tags']);
        }
    }
    ...
    public function append_author_box($content) {
        if (!get_option('earlystart_seo_show_author_box', true) || doing_filter('get_the_excerpt')) {
            return $content;
        }
