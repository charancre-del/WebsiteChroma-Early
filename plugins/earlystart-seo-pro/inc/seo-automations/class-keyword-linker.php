...
class earlystart_Keyword_Linker
{
    ...
    private function get_keywords() {
        ...
        $this->keywords = get_option('earlystart_seo_keyword_links', []);
    ...
    public function auto_link_keywords($content) {
        if (!get_option('earlystart_seo_enable_keyword_linking', true)) {
            return $content;
        }
    ...
    public function register_settings() {
        register_setting('earlystart_keyword_linking', 'earlystart_seo_enable_keyword_linking');
        register_setting('earlystart_keyword_linking', 'earlystart_seo_keyword_links');
    }
    ...
    public function render_settings_page() {
        $enabled = get_option('earlystart_seo_enable_keyword_linking', true);
        $keywords = get_option('earlystart_seo_keyword_links', []);
        ...
        <div class="wrap">
            ...
            <form method="post" action="options.php">
                <?php settings_fields('earlystart_keyword_linking'); ?>
                ...
                <table class="form-table">
                    <tr>
                        ...
                        <td>
                            <label>
                                <input type="checkbox" name="earlystart_seo_enable_keyword_linking" 
                ...
                <div id="custom-keywords">
                    ...
                    <div class="keyword-row">
                        <input type="text" name="earlystart_seo_keyword_links[<?php echo $i; ?>][keyword]" 
                            value="<?php echo esc_attr($kw['keyword']); ?>" placeholder="Keyword">
                        <input type="url" name="earlystart_seo_keyword_links[<?php echo $i; ?>][url]" 
                            value="<?php echo esc_url($kw['url']); ?>" placeholder="URL">
                        <input type="number" name="earlystart_seo_keyword_links[<?php echo $i; ?>][max]" 
        ...
        jQuery(function($) {
            ...
            $('#add-keyword').on('click', function() {
                $('#custom-keywords').append(
                    '<div class="keyword-row">' +
                    '<input type="text" name="earlystart_seo_keyword_links[' + i + '][keyword]" placeholder="Keyword">' +
                    '<input type="url" name="earlystart_seo_keyword_links[' + i + '][url]" placeholder="URL">' +
                    '<input type="number" name="earlystart_seo_keyword_links[' + i + '][max]" value="1" min="1" max="5" style="width:60px">' +
                    '<button type="button" class="button remove-keyword">×</button>' +
                    '</div>'
                );
                i++;
