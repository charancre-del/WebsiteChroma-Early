    ...
    private function generate_combo_content($program_name, $city_name, $state) {
        ...
        $response = $earlystart_llm_client->make_request([
            'messages' => [
                [
                    'role' => 'system',
                    'content' => 'You are a local SEO expert helping generate location-specific content for a pediatric therapy website focused on autism assessment, behavioral assessment, ADHD assessment, OT assessment, ABA, speech, and occupational therapy. Return valid JSON only.'
                ],
                [
                    'role' => 'user',
                    'content' => $prompt
