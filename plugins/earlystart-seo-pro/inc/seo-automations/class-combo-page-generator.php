...
class earlystart_Combo_Page_Generator
{
    ...
    public function __construct() {
        ...
        if (!get_option('earlystart_combo_sitemap_flush_Check')) {
            add_action('init', function() {
                flush_rewrite_rules();
                update_option('earlystart_combo_sitemap_flush_Check', true);
            }, 99);
        }
    }
    ...
    public function add_rewrite_rules() {
        ...
        add_rewrite_rule('^sitemap-combos\.xml$', 'index.php?earlystart_combo_sitemap=en', 'top');
        add_rewrite_rule('^sitemap-combos-es\.xml$', 'index.php?earlystart_combo_sitemap=es', 'top');
    }
    ...
    public function register_sitemap_providers() {
        wp_register_sitemap_provider('combos', new earlystart_Combo_Sitemap_Provider('en'));
        wp_register_sitemap_provider('combos-es', new earlystart_Combo_Sitemap_Provider('es'));
    }
    ...
    public function get_combo_page_html($program, $city_slug, $state, $location) {
        ...
        // This is stored in options table via earlystart_Combo_Page_Data class
        $combo_custom_data = earlystart_Combo_Page_Data::get($program->post_name, $city_slug, $state);
        ...
        // Spanish Override
        $lang = function_exists('earlystart_detect_current_language') ? earlystart_detect_current_language() : 'en';
                    ...
                    <?php else: ?>
                        ...
                        <div class="max-w-3xl mx-auto">
                            ...
                            <div class="bg-white p-8 rounded-[2rem] text-left shadow-2xl">
                                <?php echo do_shortcode('[earlystart_tour_form]'); ?>
                            </div>
                        </div>
                    <?php endif; ?>
                </div>
    ...
    public function output_schema($program, $city_name, $state, $location, $loc_address = '', $loc_zip = '', $age_range = '') {
        ...
        if (class_exists('earlystart_Schema_Registry')) {
            earlystart_Schema_Registry::register($schema, ['source' => 'combo-page-generator']);
        }
    }
    ...
    public static function get_all_cities() {
        ...
        $manual = get_option('earlystart_seo_manual_cities', []);
        foreach ($manual as $m) {
    ...
    public function handle_sitemap() {
        $lang = get_query_var('earlystart_combo_sitemap');
        if (!$lang) return;
        ...
        foreach ($combos as $combo) {
            $saved_data = earlystart_Combo_Page_Data::get($combo['program']->post_name, sanitize_title($combo['city']), $combo['state']);
    ...
    public function render_admin_page() {
        ...
        foreach ($combos as &$combo) {
            if (class_exists('earlystart_Combo_Page_Data')) {
                $combo['data'] = earlystart_Combo_Page_Data::get(
        ...
        <div class="wrap">
                    ...
                    <tr data-program="<?php echo esc_attr($program_slug); ?>" 
                        ...
                        <td>
                            <span style="color: <?php echo $status_color; ?>; font-weight: 500;">
                                <?php 
                                if (class_exists('earlystart_Combo_Page_Data')) {
                                    echo esc_html(earlystart_Combo_Page_Data::get_status_label($status));
                                } else {
            ...
            <h2>📍 Near Me Pages</h2>
            <?php if (class_exists('earlystart_Near_Me_Pages') && method_exists('earlystart_Near_Me_Pages', 'get_all_pages')): ?>
                <div style="max-height: 400px; overflow-y: auto; border: 1px solid #e5e5e5; background: #fff;">
                    <table class="wp-list-table widefat fixed striped">
                        ...
                        <tbody>
                            <?php 
                            $near_pages = earlystart_Near_Me_Pages::get_all_pages();
            ...
            <form method="post" action="options.php">
                <?php settings_fields('earlystart_auto_pages'); ?>
                <table class="form-table">
                    <tr>
                        <th>Auto-Publish AI Content</th>
                        <td>
                            <label>
                                <input type="checkbox" name="earlystart_combo_auto_publish" value="1" <?php checked($auto_publish); ?>>
                    ...
                    <tr>
                        <th>Manual Cities</th>
                        <td>
                            <textarea name="earlystart_seo_manual_cities_raw" rows="5" class="large-text"><?php 
                                $manual = get_option('earlystart_seo_manual_cities', []);
                                foreach ($manual as $m) {
        ...
        jQuery(function($) {
            ...
            $('.edit-combo-btn').on('click', function() {
                ...
                $.post(ajaxurl, {
                    action: 'earlystart_combo_get_data',
                    nonce: nonce,
            ...
            $('#save-combo-btn').on('click', function() {
                ...
                $.post(ajaxurl, {
                    action: 'earlystart_combo_save_data',
                    nonce: nonce,
            ...
            $('.ai-generate-btn, #ai-generate-modal-btn').on('click', function() {
                ...
                $.post(ajaxurl, {
                    action: 'earlystart_combo_ai_generate',
                    nonce: nonce,
                    program_slug: program,
                    city_slug: city,
                    state: state
            ...
            $('#do-bulk-action').on('click', function() {
                ...
                if (action === 'ai_generate' || action === 'ai_translate') {
                    var ajax_action = action === 'ai_translate' ? 'earlystart_combo_ai_bulk_translate' : 'earlystart_combo_ai_bulk_generate';
                    ...
                    $.post(ajaxurl, {
                        action: 'earlystart_combo_bulk_status',
                        nonce: nonce,
                        combos: combos,
                        status: status
                    }, function(response) {
...
add_action('admin_init', function() {
    // Auto-publish toggle
    register_setting('earlystart_auto_pages', 'earlystart_combo_auto_publish');
    
    // Manual cities
    register_setting('earlystart_auto_pages', 'earlystart_seo_manual_cities_raw', function($raw) {
        $cities = [];
        $lines = explode("\n", $raw);
        ...
        update_option('earlystart_seo_manual_cities', $cities);
        return $raw;
    });
});
 ...
 */
class earlystart_Combo_Sitemap_Provider extends WP_Sitemaps_Provider {
    private $lang;
