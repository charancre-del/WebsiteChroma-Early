    ...
    private function render_related_locations($locations)
    {
        ...
        <section class="chroma-related-locations py-16 border-t border-brand-ink/5 bg-brand-cream/30 mt-16">
            <div class="max-w-7xl mx-auto px-4 lg:px-6">
                <h2 class="font-serif text-3xl font-bold text-brand-ink mb-10">
                    <?php _e('Other Locations Near You', 'chroma-excellence'); ?></h2>
                <div class="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
                    <?php foreach ($locations as $loc):
                        ...
                        <div class="location-card group">
                            <div
                                class="bg-white rounded-[2rem] p-6 shadow-card border border-brand-ink/5 hover:border-<?php echo esc_attr($colors['border']); ?>/30 transition-all hover:-translate-y-1 h-full flex flex-col relative overflow-hidden">
                                ...
                                <h3
                                    class="font-serif text-xl font-bold text-brand-ink mb-2 group-hover:text-<?php echo esc_attr($colors['text']); ?> transition-colors">
                                    <a href="<?php echo get_permalink($loc); ?>"><?php echo esc_html($loc->post_title); ?></a>
                                </h3>

                                <?php if ($city): ?>
                                    <p class="text-sm text-brand-ink/80 mb-4">Pediatric Therapy in <?php echo esc_html($city); ?></p>
                                <?php endif; ?>

                                <div class="flex flex-wrap gap-2 mb-4">
                                    <span
                                        class="inline-flex items-center gap-1.5 px-3 py-1 bg-chroma-blueLight/50 text-chroma-blueDark text-[10px] font-bold uppercase rounded-full border border-chroma-blueDark/10">
                                        <i class="fa-solid fa-heart-pulse"></i> Pediatric Therapy
                                    </span>
                                    <span
                                        class="inline-flex items-center gap-1.5 px-3 py-1 bg-chroma-yellowLight/50 text-chroma-yellowDark text-[10px] font-bold uppercase rounded-full border border-chroma-yellowDark/10">
                                        <i class="fa-solid fa-check"></i> Assessment Support
                                    </span>
                                </div>
                                ...
                                <div class="mt-auto flex items-center justify-between">
                                    <?php if ($distance !== null): ?>
                                        <span
                                            class="text-[11px] font-bold text-chroma-blue uppercase tracking-wider"><?php echo $distance; ?>
                                            miles away</span>
                                    <?php endif; ?>
                                    <a href="<?php echo get_permalink($loc); ?>"
                                        class="text-[10px] font-bold text-brand-ink/40 uppercase tracking-widest group-hover:text-<?php echo esc_attr($colors['text']); ?> transition-colors">
