...
class earlystart_Link_Equity_Analyzer
{
    ...
    private static function find_suggested_sources($target, $all_pages) {
        ...
        foreach ($all_pages as $page) {
            ...
            if (isset($GLOBALS['earlystart_link_scores'][$page->ID])) {
                $relevance += min(0.2, $GLOBALS['earlystart_link_scores'][$page->ID] / 50);
            }
    ...
    public function render_dashboard() {
        ...
        $nonce = wp_create_nonce('earlystart_link_equity');
        ?>
        <div class="wrap">
        ...
        jQuery(function($) {
            ...
            $('#ai-fix-preview-btn').on('click', function() {
                ...
                $.post(ajaxurl, {
                    action: 'earlystart_link_equity_ai_preview',
                    nonce: nonce,
                    source_id: sourceId,
            ...
            $('#ai-fix-apply-btn').on('click', function() {
                ...
                $.post(ajaxurl, {
                    action: 'earlystart_link_equity_ai_apply',
                    nonce: nonce,
                    source_id: sourceId,
                    target_id: currentTarget.id,
                    target_title: currentTarget.title,
...
add_action('wp_ajax_earlystart_link_equity_ai_preview', function() {
    check_ajax_referer('earlystart_link_equity', 'nonce');
...
add_action('wp_ajax_earlystart_link_equity_ai_apply', function() {
    check_ajax_referer('earlystart_link_equity', 'nonce');
...
new earlystart_Link_Equity_Analyzer();
