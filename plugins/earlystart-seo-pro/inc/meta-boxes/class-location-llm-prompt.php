...
class earlystart_Location_LLM_Prompt_Meta_Box extends earlystart_Advanced_SEO_Meta_Box_Base
{
    public function get_id()
    {
        return 'earlystart_location_llm_prompt';
    }
    ...
    public function render_fields($post)
    {
        ...
        // Get fallback
        $fallback_description = earlystart_Fallback_Resolver::get_llm_description($post->ID);
    ...
    public function save_fields($post_id)
    {
        ...
        if (isset($_POST['seo_llm_description'])) {
            $description = earlystart_Field_Sanitizer::sanitize_textarea($_POST['seo_llm_description']);
            update_post_meta($post_id, 'seo_llm_description', $description);
        }

        // Save when to recommend
        if (isset($_POST['seo_llm_when_to_recommend'])) {
            $scenarios = earlystart_Field_Sanitizer::sanitize_text_array($_POST['seo_llm_when_to_recommend']);
