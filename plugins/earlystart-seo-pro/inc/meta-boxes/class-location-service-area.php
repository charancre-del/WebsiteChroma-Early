...
class earlystart_Location_Service_Area_Meta_Box extends earlystart_Advanced_SEO_Meta_Box_Base
{
    public function get_id()
    {
        return 'earlystart_location_service_area';
    }
    ...
    public function render_fields($post)
    {
        ...
        // Get fallback data
        $fallback_circle = earlystart_Fallback_Resolver::get_service_area_circle($post->ID);
        $fallback_cities = earlystart_Fallback_Resolver::get_service_area_cities($post->ID);
    ...
    public function save_fields($post_id)
    {
        ...
        if (isset($_POST['seo_llm_service_area_lat'])) {
            $lat = earlystart_Field_Sanitizer::sanitize_latitude($_POST['seo_llm_service_area_lat']);
            update_post_meta($post_id, 'seo_llm_service_area_lat', $lat);
        }

        // Save longitude
        if (isset($_POST['seo_llm_service_area_lng'])) {
            $lng = earlystart_Field_Sanitizer::sanitize_longitude($_POST['seo_llm_service_area_lng']);
            update_post_meta($post_id, 'seo_llm_service_area_lng', $lng);
        }

        // Save radius
        if (isset($_POST['seo_llm_service_area_radius'])) {
            $radius = earlystart_Field_Sanitizer::sanitize_number($_POST['seo_llm_service_area_radius']);
            update_post_meta($post_id, 'seo_llm_service_area_radius', $radius);
        }

        // Save cities
        if (isset($_POST['seo_llm_service_area_cities'])) {
            $cities = earlystart_Field_Sanitizer::sanitize_text_array($_POST['seo_llm_service_area_cities']);
        ...
        if (isset($_POST['seo_llm_service_area_state'])) {
            $state = earlystart_Field_Sanitizer::sanitize_text($_POST['seo_llm_service_area_state']);
