...
class earlystart_LLM_Context_Builder
{
    /**
    ...
    public static function build($location_id)
    {
        ...
        // Target queries
        $queries = earlystart_Fallback_Resolver::get_llm_target_queries($location_id);
        if (!empty($queries)) {
            $context['target_queries'] = $queries;
        }

        // Key differentiators
        $differentiators = earlystart_Fallback_Resolver::get_llm_key_differentiators($location_id);
    ...
    public static function build_prompt_fields($location_id)
    {
        ...
        // Description
        $description = earlystart_Fallback_Resolver::get_llm_description($location_id);
        if ($description) {
        ...
        // Comparison factors
        $factors = earlystart_Fallback_Resolver::get_comparison_factors($location_id);
        if (!empty($factors)) {
