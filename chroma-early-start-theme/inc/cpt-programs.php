 ...
 */
function earlystart_register_program_cpt()
{
	$labels = array(
		'name' => _x('Programs', 'Post Type General Name', 'chroma-early-start'),
		'singular_name' => _x('Program', 'Post Type Singular Name', 'chroma-early-start'),
		'menu_name' => __('Programs', 'chroma-early-start'),
		'all_items' => __('All Programs', 'chroma-early-start'),
		'add_new_item' => __('Add New Program', 'chroma-early-start'),
		'edit_item' => __('Edit Program', 'chroma-early-start'),
		'view_item' => __('View Program', 'chroma-early-start'),
	);
	...
	$args = array(
		'label' => __('Program', 'chroma-early-start'),
		'labels' => $labels,
 ...
 */
function earlystart_program_admin_columns($columns)
{
	$new_columns = array();
	$new_columns['cb'] = $columns['cb'];
	$new_columns['title'] = $columns['title'];
	$new_columns['age_range'] = __('Age Range', 'chroma-early-start');
	$new_columns['locations'] = __('Locations', 'chroma-early-start');
	$new_columns['date'] = $columns['date'];

	return $new_columns;
}
add_filter('manage_program_posts_columns', 'earlystart_program_admin_columns');

/**
 * Populate admin columns
 */
function earlystart_program_admin_column_content($column, $post_id)
{
	switch ($column) {
		case 'age_range':
...
add_action('manage_program_posts_custom_column', 'earlystart_program_admin_column_content', 10, 2);

/**
 * Custom title placeholder
 */
function earlystart_program_title_placeholder($title)
{
	$screen = get_current_screen();
	if ('program' === $screen->post_type) {
		$title = __('e.g., Infant Care', 'chroma-early-start');
	}
	return $title;
}
add_filter('enter_title_here', 'earlystart_program_title_placeholder');

/**
 * Register meta fields for Program anchors and SEO intro
 */
function earlystart_register_program_meta()
{
	$meta_args = array(
...
add_action('init', 'earlystart_register_program_meta');

/**
 * Add meta box for anchor and SEO intro fields
 */
function earlystart_program_meta_box()
{
	add_meta_box(
		'chroma-program-anchor-seo',
		__('Program Anchor & SEO Intro', 'chroma-early-start'),
		'earlystart_program_meta_box_render',
		'program',
		'side',
		'default'
	);
}
add_action('add_meta_boxes', 'earlystart_program_meta_box');

/**
 * Render the meta box fields
 */
function earlystart_program_meta_box_render($post)
{
	wp_nonce_field('earlystart_program_meta_nonce', 'earlystart_program_meta_nonce_field');
	...
	<p>
		<label for="program_anchor_slug"
			class="screen-reader-text"><?php esc_html_e('Program Anchor', 'chroma-early-start'); ?></label>
		<input type="text" id="program_anchor_slug" name="program_anchor_slug" value="<?php echo esc_attr($anchor); ?>"
			class="widefat" placeholder="<?php esc_attr_e('e.g., infant', 'chroma-early-start'); ?>" />
		<small><?php esc_html_e('Used for #anchors and homepage wizard links. Defaults to the slug.', 'chroma-early-start'); ?></small>
	</p>
	<p>
		<label for="program_seo_heading"
			class="screen-reader-text"><?php esc_html_e('SEO Heading', 'chroma-early-start'); ?></label>
		<input type="text" id="program_seo_heading" name="program_seo_heading" value="<?php echo esc_attr($heading); ?>"
			class="widefat" placeholder="<?php esc_attr_e('Program intro heading', 'chroma-early-start'); ?>" />
	</p>
	<p>
		<label for="program_seo_summary"
			class="screen-reader-text"><?php esc_html_e('SEO Summary', 'chroma-early-start'); ?></label>
		<textarea id="program_seo_summary" name="program_seo_summary" class="widefat" rows="3"
			placeholder="<?php esc_attr_e('Short overview that lives above the card', 'chroma-early-start'); ?>"><?php echo esc_textarea($summary); ?></textarea>
	</p>
	<p>
		<label for="program_seo_highlights"
			class="screen-reader-text"><?php esc_html_e('SEO Highlights', 'chroma-early-start'); ?></label>
		<textarea id="program_seo_highlights" name="program_seo_highlights" class="widefat" rows="4"
			placeholder="<?php esc_attr_e("One bullet per line (e.g. ratios, curriculum)", 'chroma-early-start'); ?>"><?php echo esc_textarea($highlights); ?></textarea>
	</p>
	<hr />
	<p>
		<label for="program_meta_title"
			class="screen-reader-text"><?php esc_html_e('Meta Title', 'chroma-early-start'); ?></label>
		<input type="text" id="program_meta_title" name="program_meta_title" value="<?php echo esc_attr($meta_title); ?>"
			class="widefat" placeholder="<?php esc_attr_e('Custom title tag (optional)', 'chroma-early-start'); ?>" />
		<small><?php esc_html_e('Used on the program detail for search visibility.', 'chroma-early-start'); ?></small>
	</p>
	<p>
		<label for="program_meta_description"
			class="screen-reader-text"><?php esc_html_e('Meta Description', 'chroma-early-start'); ?></label>
		<textarea id="program_meta_description" name="program_meta_description" class="widefat" rows="3"
			placeholder="<?php esc_attr_e('1–2 sentence description for search snippets', 'chroma-early-start'); ?>"><?php echo esc_textarea($meta_desc); ?></textarea>
	</p>
	<p>
		<label for="program_faq_items"
			class="screen-reader-text"><?php esc_html_e('FAQ Items', 'chroma-early-start'); ?></label>
		<textarea id="program_faq_items" name="program_faq_items" class="widefat" rows="4"
			placeholder="<?php esc_attr_e('Question | Answer (one per line)', 'chroma-early-start'); ?>"><?php echo esc_textarea($faq_items); ?></textarea>
		<small><?php esc_html_e('Populate FAQ schema and on-page Q&A.', 'chroma-early-start'); ?></small>
	</p>
	<?php
}
 ...
 */
function earlystart_program_meta_box_save($post_id)
{
	if (!isset($_POST['earlystart_program_meta_nonce_field']) || !wp_verify_nonce(wp_unslash($_POST['earlystart_program_meta_nonce_field']), 'earlystart_program_meta_nonce')) {
		return;
	}
...
add_action('save_post', 'earlystart_program_meta_box_save');

/**
 * Add meta box for program locations
 */
function earlystart_program_locations_meta_box()
{
	add_meta_box(
		'chroma-program-locations',
		__('Available at Locations', 'chroma-early-start'),
		'earlystart_program_locations_meta_box_render',
		'program',
		'side',
		'default'
	);
	...
	add_meta_box(
		...
		__('Program Details', 'chroma-early-start'),
		'earlystart_program_details_meta_box_render',
		'program',
		'normal',
		'high'
	);
	...
	add_meta_box(
		...
		__('Single Page Content', 'chroma-early-start'),
		'earlystart_program_single_page_meta_box_render',
		'program',
		'normal',
		'default'
	);
}
add_action('add_meta_boxes', 'earlystart_program_locations_meta_box');

/**
 * Render program locations meta box
 */
function earlystart_program_locations_meta_box_render($post)
{
	wp_nonce_field('earlystart_program_locations_nonce', 'earlystart_program_locations_nonce_field');
	...
	?>
	<p><?php _e('Select the locations where this program is available:', 'chroma-early-start'); ?></p>
	<p style="margin-bottom: 10px;">
		<button type="button" id="chroma-toggle-all-locations" class="button button-secondary" style="margin-bottom: 5px;">
			<?php _e('Select All / Deselect All', 'chroma-early-start'); ?>
		</button>
	</p>
	<div id="chroma-locations-list"
		...
		<?php if (!empty($all_locations)): ?>
			...
			<?php endforeach; ?>
		<?php else: ?>
			<p><?php _e('No locations found. Please add locations first.', 'chroma-early-start'); ?></p>
		<?php endif; ?>
	</div>
	<p><small><?php _e('This program will only appear on selected location pages.', 'chroma-early-start'); ?></small></p>
 ...
 */
function earlystart_program_locations_meta_box_save($post_id)
{
	// Verify nonce
	if (!isset($_POST['earlystart_program_locations_nonce_field']) || !wp_verify_nonce(wp_unslash($_POST['earlystart_program_locations_nonce_field']), 'earlystart_program_locations_nonce')) {
		return;
	}
...
add_action('save_post_program', 'earlystart_program_locations_meta_box_save');

/**
 * Render program details meta box
 */
function earlystart_program_details_meta_box_render($post)
{
	wp_nonce_field('earlystart_program_details_nonce', 'earlystart_program_details_nonce_field');
	...
	<div class="chroma-program-field">
		<label for="program_icon"><?php _e('Program Icon/Emoji', 'chroma-early-start'); ?></label>
		...
		<small><?php _e('Select a preset or type a custom emoji.', 'chroma-early-start'); ?></small>
	...
	<div class="chroma-program-field">
		<label for="program_age_range"><?php _e('Age Range', 'chroma-early-start'); ?></label>
		...
		<small><?php _e('Age range badge shown on program card (e.g., "6w - 12mo", "1 Year", "4yr - 5yr")', 'chroma-early-start'); ?></small>
	</div>

	<div class="chroma-program-field">
		<label for="program_features"><?php _e('Program Features', 'chroma-early-start'); ?></label>
		...
		<small><?php _e('Enter one feature per line. These will display with checkmarks on the program card.', 'chroma-early-start'); ?></small>
	</div>

	<div class="chroma-program-field">
		<label for="program_cta_text"><?php _e('CTA Button Text', 'chroma-early-start'); ?></label>
		...
		<small><?php _e('Text for the call-to-action button (default: "Schedule Tour")', 'chroma-early-start'); ?></small>
	</div>

	<div class="chroma-program-field">
		<label for="program_cta_link"><?php _e('CTA Button Link', 'chroma-early-start'); ?></label>
		<input type="text" id="program_cta_link" name="program_cta_link" value="<?php echo esc_attr($cta_link); ?>"
			placeholder="#tour" />
		<small><?php _e('URL or anchor link for the CTA button (default: "#tour")', 'chroma-early-start'); ?></small>
	</div>

	<div class="chroma-program-field">
		<label for="program_lesson_plan_file"><?php _e('Lesson Plan File (URL)', 'chroma-early-start'); ?></label>
		...
		<small><?php _e('Paste the full URL to the PDF lesson plan. If set, a button will appear on the program page.', 'chroma-early-start'); ?></small>
	</div>

	<div class="chroma-program-field">
		<label for="program_color_scheme"><?php _e('Color Scheme', 'chroma-early-start'); ?></label>
		...
		<small><?php _e('Color theme for the program card hover effects and badges', 'chroma-early-start'); ?></small>
	</div>

	<p><strong><?php _e('Note:', 'chroma-early-start'); ?></strong>
		<?php _e('The program description shown on the card comes from the "Excerpt" field. The featured image is used as the card image.', 'chroma-early-start'); ?>
	</p>
	<?php
}
 ...
 */
function earlystart_program_details_meta_box_save($post_id)
{
	// Verify nonce
	if (!isset($_POST['earlystart_program_details_nonce_field']) || !wp_verify_nonce(wp_unslash($_POST['earlystart_program_details_nonce_field']), 'earlystart_program_details_nonce')) {
		return;
	}
...
add_action('save_post_program', 'earlystart_program_details_meta_box_save');

/**
 * Render single page content meta box
 */
function earlystart_program_single_page_meta_box_render($post)
{
	wp_nonce_field('earlystart_program_single_page_nonce', 'earlystart_program_single_page_nonce_field');
	...
	<div class="chroma-single-field">
		<label for="program_hero_title"><?php _e('Hero Title', 'chroma-early-start'); ?></label>
		...
		<small><?php _e('Main heading on single program page (defaults to program title if empty)', 'chroma-early-start'); ?></small>
	</div>

	<div class="chroma-single-field">
		<label for="program_hero_description"><?php _e('Hero Description', 'chroma-early-start'); ?></label>
		...
		<small><?php _e('Description paragraph in hero section', 'chroma-early-start'); ?></small>
	</div>
	...
	<div class="chroma-single-field">
		<label for="program_prism_title"><?php _e('Prism Section Title', 'chroma-early-start'); ?></label>
		...
		<small><?php _e('Title for the Prismpath focus section', 'chroma-early-start'); ?></small>
	</div>

	<div class="chroma-single-field">
		<label for="program_prism_description"><?php _e('Prism Description', 'chroma-early-start'); ?></label>
		...
		<small><?php _e('Description explaining the program\'s Prismpath focus', 'chroma-early-start'); ?></small>
	</div>

	<div class="chroma-single-field">
		<label><?php _e('Prismpath Chart Values (0-100)', 'chroma-early-start'); ?></label>
		...
		<small><?php _e('Set values 0-100 for each pillar. These create the radar chart.', 'chroma-early-start'); ?></small>
	...
	<div class="chroma-single-field">
		<label for="program_prism_focus_items"><?php _e('Focus Items', 'chroma-early-start'); ?></label>
		...
		<small><?php _e('Bullet points explaining the focus. One per line.', 'chroma-early-start'); ?></small>
	</div>
	...
	<div class="chroma-single-field">
		<label for="program_schedule_title"><?php _e('Schedule Section Title', 'chroma-early-start'); ?></label>
		...
		<small><?php _e('Title for the schedule section', 'chroma-early-start'); ?></small>
	</div>

	<div class="chroma-single-field">
		<label for="program_schedule_items"><?php _e('Schedule Items', 'chroma-early-start'); ?></label>
		...
		<small><?php _e('Format: Badge|Title|Description (one per line). Badge can be AM, Mid, PM, or any text.', 'chroma-early-start'); ?></small>
	</div>

	<p><strong><?php _e('Note:', 'chroma-early-start'); ?></strong>
		<?php _e('The featured image is used as the hero image on the single program page.', 'chroma-early-start'); ?></p>
	<?php
}
 ...
 */
function earlystart_program_single_page_meta_box_save($post_id)
{
	// Verify nonce
	if (!isset($_POST['earlystart_program_single_page_nonce_field']) || !wp_verify_nonce(wp_unslash($_POST['earlystart_program_single_page_nonce_field']), 'earlystart_program_single_page_nonce')) {
		return;
	}
...
add_action('save_post_program', 'earlystart_program_single_page_meta_box_save');
