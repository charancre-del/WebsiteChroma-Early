 ...
 */
function earlystart_register_schema_meta_boxes() {
	// Homepage/Organization Schema
	add_meta_box(
		'chroma-schema-organization',
		__( 'Schema.org - Organization (ChildCare)', 'chroma-early-start' ),
	...
	add_meta_box(
		...
		__( 'Schema.org - LocalBusiness (ChildCare)', 'chroma-early-start' ),
	...
	add_meta_box(
		...
		__( 'Schema.org - Service', 'chroma-early-start' ),
		'earlystart_schema_program_meta_box',
		'program',
		'normal',
		'high'
	);
}
add_action( 'add_meta_boxes', 'earlystart_register_schema_meta_boxes' );

/**
 * Organization Schema Meta Box (Homepage)
 */
function earlystart_schema_organization_meta_box( $post ) {
	...
	wp_nonce_field( 'earlystart_schema_organization', 'earlystart_schema_organization_nonce' );
 ...
 */
function earlystart_schema_location_meta_box( $post ) {
	wp_nonce_field( 'earlystart_schema_location', 'earlystart_schema_location_nonce' );
 ...
 */
function earlystart_schema_program_meta_box( $post ) {
	wp_nonce_field( 'earlystart_schema_program', 'earlystart_schema_program_nonce' );
 ...
 */
function earlystart_save_schema_organization_meta( $post_id ) {
	// Security checks
	if ( ! isset( $_POST['earlystart_schema_organization_nonce'] ) ) {
		return;
	}

	if ( ! wp_verify_nonce( $_POST['earlystart_schema_organization_nonce'], 'earlystart_schema_organization' ) ) {
		return;
	}
...
add_action( 'save_post_page', 'earlystart_save_schema_organization_meta' );

/**
 * Save Location Schema Meta
 */
function earlystart_save_schema_location_meta( $post_id ) {
	// Security checks
	if ( ! isset( $_POST['earlystart_schema_location_nonce'] ) ) {
		return;
	}

	if ( ! wp_verify_nonce( $_POST['earlystart_schema_location_nonce'], 'earlystart_schema_location' ) ) {
		return;
	}
...
add_action( 'save_post_location', 'earlystart_save_schema_location_meta' );

/**
 * Save Program Schema Meta
 */
function earlystart_save_schema_program_meta( $post_id ) {
	// Security checks
	if ( ! isset( $_POST['earlystart_schema_program_nonce'] ) ) {
		return;
	}

	if ( ! wp_verify_nonce( $_POST['earlystart_schema_program_nonce'], 'earlystart_schema_program' ) ) {
		return;
	}
...
add_action( 'save_post_program', 'earlystart_save_schema_program_meta' );
