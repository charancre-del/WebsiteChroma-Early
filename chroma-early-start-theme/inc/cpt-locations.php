 ...
 */
function earlystart_register_location_cpt()
{
	$labels = array(
		'name' => _x('Locations', 'Post Type General Name', 'chroma-early-start'),
		'singular_name' => _x('Location', 'Post Type Singular Name', 'chroma-early-start'),
		'menu_name' => __('Locations', 'chroma-early-start'),
		'all_items' => __('All Locations', 'chroma-early-start'),
		'add_new_item' => __('Add New Location', 'chroma-early-start'),
		'edit_item' => __('Edit Location', 'chroma-early-start'),
		'view_item' => __('View Location', 'chroma-early-start'),
	);

	$args = array(
		'label' => __('Location', 'chroma-early-start'),
		'labels' => $labels,
...
{
	$labels = array(
		'name' => _x('Location Regions', 'taxonomy general name', 'chroma-early-start'),
		'singular_name' => _x('Location Region', 'taxonomy singular name', 'chroma-early-start'),
		'search_items' => __('Search Location Regions', 'chroma-early-start'),
		'all_items' => __('All Location Regions', 'chroma-early-start'),
		'parent_item' => __('Parent Region', 'chroma-early-start'),
		'parent_item_colon' => __('Parent Region:', 'chroma-early-start'),
		'edit_item' => __('Edit Region', 'chroma-early-start'),
		'update_item' => __('Update Region', 'chroma-early-start'),
		'add_new_item' => __('Add New Region', 'chroma-early-start'),
		'new_item_name' => __('New Region Name', 'chroma-early-start'),
		'menu_name' => __('Location Regions', 'chroma-early-start'),
	);
	...
	register_taxonomy(
		...
		array(
			...
			'rewrite' => false,
			'default_term' => array(
				'name' => __('Uncategorized Locations', 'chroma-early-start'),
				'slug' => 'uncategorized-locations',
			),
		)
	);
}
add_action('init', 'earlystart_register_location_taxonomy', 1);

/**
 * Add admin columns
 */
function earlystart_location_admin_columns($columns)
{
	$new_columns = array();
	$new_columns['cb'] = $columns['cb'];
	$new_columns['title'] = $columns['title'];
	$new_columns['city'] = __('City', 'chroma-early-start');
	$new_columns['state'] = __('State', 'chroma-early-start');
	$new_columns['phone'] = __('Phone', 'chroma-early-start');
	$new_columns['capacity'] = __('Capacity', 'chroma-early-start');
	$new_columns['date'] = $columns['date'];

	return $new_columns;
}
add_filter('manage_location_posts_columns', 'earlystart_location_admin_columns');
...
add_action('manage_location_posts_custom_column', 'earlystart_location_admin_column_content', 10, 2);

/**
 * Make columns sortable
 */
function earlystart_location_sortable_columns($columns)
{
	$columns['city'] = 'city';
	$columns['state'] = 'state';
	return $columns;
}
add_filter('manage_edit-location_sortable_columns', 'earlystart_location_sortable_columns');

/**
 * Custom title placeholder
 */
function earlystart_location_title_placeholder($title)
{
	$screen = get_current_screen();
	if ('location' === $screen->post_type) {
		$title = __('e.g., Johns Creek Campus', 'chroma-early-start');
	}
	return $title;
}
add_filter('enter_title_here', 'earlystart_location_title_placeholder');

/**
 * Add custom fields to location_region taxonomy
 */
function earlystart_location_region_add_form_fields()
{
	?>
	<div class="form-field">
		<label for="region_color_bg"><?php _e('Background Color Class', 'chroma-early-start'); ?></label>
		<input type="text" name="region_color_bg" id="region_color_bg" value="chroma-greenLight"
			placeholder="e.g., chroma-greenLight">
		<p class="description">
			<?php _e('Tailwind background color class (e.g., chroma-greenLight, chroma-redLight, chroma-blueLight, chroma-yellowLight, chroma-purpleLight, chroma-orangeLight, chroma-tealLight)', 'chroma-early-start'); ?>
		</p>
	</div>
	<div class="form-field">
		<label for="region_color_text"><?php _e('Text Color Class', 'chroma-early-start'); ?></label>
		<input type="text" name="region_color_text" id="region_color_text" value="chroma-green"
			placeholder="e.g., chroma-green">
		<p class="description">
			<?php _e('Tailwind text color class (e.g., chroma-green, chroma-red, chroma-blue, chroma-yellow, chroma-purple, chroma-orange, chroma-teal)', 'chroma-early-start'); ?>
		</p>
	</div>
	<div class="form-field">
		<label for="region_color_border"><?php _e('Border Color Class', 'chroma-early-start'); ?></label>
		<input type="text" name="region_color_border" id="region_color_border" value="chroma-green"
			placeholder="e.g., chroma-green">
		<p class="description">
			<?php _e('Tailwind border color class (e.g., chroma-green, chroma-red, chroma-blue, chroma-yellow, chroma-purple, chroma-orange, chroma-teal)', 'chroma-early-start'); ?>
		</p>
	</div>
	<?php
}
add_action('location_region_add_form_fields', 'earlystart_location_region_add_form_fields');

/**
 * Add custom fields to location_region taxonomy edit form
 */
function earlystart_location_region_edit_form_fields($term)
{
	...
	?>
	<tr class="form-field">
		<th scope="row"><label for="region_color_bg"><?php _e('Background Color Class', 'chroma-early-start'); ?></label>
		</th>
		<td>
	...
	<tr class="form-field">
		<th scope="row"><label for="region_color_text"><?php _e('Text Color Class', 'chroma-early-start'); ?></label></th>
		<td>
	...
	<tr class="form-field">
		<th scope="row"><label for="region_color_border"><?php _e('Border Color Class', 'chroma-early-start'); ?></label>
		</th>
		<td>
...
add_action('location_region_edit_form_fields', 'earlystart_location_region_edit_form_fields');

/**
 * Save location_region taxonomy custom fields
 */
function earlystart_save_location_region_meta($term_id)
{
...
add_action('created_location_region', 'earlystart_save_location_region_meta');
add_action('edited_location_region', 'earlystart_save_location_region_meta');

/**
 * Add meta box for location custom fields
 */
function earlystart_location_custom_fields_meta_box()
{
	add_meta_box(
		'chroma-location-custom-fields',
		__('Location Details', 'chroma-early-start'),
		'earlystart_render_location_custom_fields_meta_box',
		'location',
		'normal',
		'high'
	);
}
add_action('add_meta_boxes', 'earlystart_location_custom_fields_meta_box');

/**
 * Render location custom fields meta box
 */
function earlystart_render_location_custom_fields_meta_box($post)
{
	wp_nonce_field('earlystart_location_meta_nonce', 'earlystart_location_meta_nonce_field');
	...
	<div class="chroma-meta-section">
		<h4><?php _e('Hero Section', 'chroma-early-start'); ?></h4>

		<div class="chroma-meta-field">
			<label for="location_hero_subtitle"><?php _e('Hero Subtitle', 'chroma-early-start'); ?></label>
			...
			<small><?php _e('Small badge text shown above the location name', 'chroma-early-start'); ?></small>
		</div>

		<div class="chroma-meta-field">
			<label for="location_hero_review_text"><?php _e('Hero Review Text', 'chroma-early-start'); ?></label>
			...
			<small><?php _e('Review text displayed in the floating badge (leave empty to hide)', 'chroma-early-start'); ?></small>
		</div>

		<div class="chroma-meta-field">
			<label for="location_hero_review_author"><?php _e('Hero Review Author', 'chroma-early-start'); ?></label>
			...
			<small><?php _e('Author of the review (defaults to "Parent Review" if empty)', 'chroma-early-start'); ?></small>
		</div>

		<div class="chroma-meta-field">
			<label for="location_hero_gallery"><?php _e('Hero Image Gallery (URLs)', 'chroma-early-start'); ?></label>
			...
			<small><?php _e('Enter image URLs (one per line). If provided, these will display as a carousel in the hero section. Leave empty to use the featured image.', 'chroma-early-start'); ?></small>
		</div>

		<div class="chroma-meta-field">
			<label for="location_virtual_tour_embed"><?php _e('Virtual Tour Embed Code', 'chroma-early-start'); ?></label>
			...
			<small><?php _e('Paste the full embed code for your virtual tour. If provided, this will display on the location page. If empty, no space will be created on the page.', 'chroma-early-start'); ?></small>
		</div>

		<div class="chroma-meta-field">
			<label for="location_tagline"><?php _e('Tagline', 'chroma-early-start'); ?></label>
			...
			<small><?php _e('Main tagline for this location (last 2 words will be italicized)', 'chroma-early-start'); ?></small>
		</div>

		<div class="chroma-meta-field">
			<label for="location_description"><?php _e('Description', 'chroma-early-start'); ?></label>
			...
			<small><?php _e('Brief description shown in hero section', 'chroma-early-start'); ?></small>
		</div>
	</div>

	<div class="chroma-meta-section">
		<h4><?php _e('Location Stats', 'chroma-early-start'); ?></h4>

		<div class="chroma-meta-field">
			<label for="location_ages_served"><?php _e('Ages Served', 'chroma-early-start'); ?></label>
			...
			<small><?php _e('Age range served at this location', 'chroma-early-start'); ?></small>
		</div>

		<div class="chroma-meta-field">
			<label for="location_special_programs"><?php _e('Special Programs (Badges)', 'chroma-early-start'); ?></label>
			...
			<small><?php _e('Enter programs separated by commas. These appear as badges on the location card.', 'chroma-early-start'); ?></small>
		</div>
	...
	<div class="chroma-meta-field">
		<label for="location_quality_rated">
			...
			<?php _e('Quality Rated by Georgia DECAL', 'chroma-early-start'); ?>
		</label>
		<small><?php _e('Check if this location has achieved Georgia\'s Quality Rated status', 'chroma-early-start'); ?></small>
	</div>

	<div class="chroma-meta-field">
		<label for="_earlystart_license_number"><?php _e('DECAL License Number', 'chroma-early-start'); ?></label>
		...
		<small><?php _e('Enter the official Georgia DECAL license number for this facility', 'chroma-early-start'); ?></small>
	</div>
		<div class="chroma-meta-field">
			<label for="location_google_rating"><?php _e('Google Rating', 'chroma-early-start'); ?></label>
			...
			<small><?php _e('Google rating for this location (e.g., 4.9)', 'chroma-early-start'); ?></small>
		</div>

		<div class="chroma-meta-field">
			<label for="location_hours"><?php _e('Hours', 'chroma-early-start'); ?></label>
			<input type="text" id="location_hours" name="location_hours" value="<?php echo esc_attr($hours); ?>"
				placeholder="e.g., 7am - 6pm" />
			<small><?php _e('Operating hours (Mon-Fri)', 'chroma-early-start'); ?></small>
		</div>
	</div>

	<div class="chroma-meta-section">
		<h4><?php _e('Campus Director', 'chroma-early-start'); ?></h4>

		<div class="chroma-meta-field">
			<label for="location_director_name"><?php _e('Director Name', 'chroma-early-start'); ?></label>
			...
			<small><?php _e('Name of the campus director (leave empty to hide director section)', 'chroma-early-start'); ?></small>
		</div>

		<div class="chroma-meta-field">
			<label for="location_director_heading"><?php _e('Director Section Heading', 'chroma-early-start'); ?></label>
			...
			<small><?php _e('Headline for the director section (fallback: Welcome to Chroma [City].)', 'chroma-early-start'); ?></small>
		</div>

		<div class="chroma-meta-field">
			<label for="location_director_bio"><?php _e('Director Bio', 'chroma-early-start'); ?></label>
			...
			<small><?php _e('Brief bio/welcome message from the director', 'chroma-early-start'); ?></small>
		</div>

		<div class="chroma-meta-field">
			<label for="location_director_photo"><?php _e('Director Photo', 'chroma-early-start'); ?></label>
			...
			<small><?php _e('Director photo (optional)', 'chroma-early-start'); ?></small>
		</div>

		<div class="chroma-meta-field">
			<label for="location_director_signature"><?php _e('Director Signature Image', 'chroma-early-start'); ?></label>
			...
			<div class="chroma-image-preview"></div>
			<small><?php _e('Director signature image (optional)', 'chroma-early-start'); ?></small>
		</div>
	</div>

	<div class="chroma-meta-section">
		<h4><?php _e('Address & Contact', 'chroma-early-start'); ?></h4>

		<div class="chroma-meta-field">
			<label for="location_address"><?php _e('Street Address', 'chroma-early-start'); ?></label>
		...
		<div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px;">
			<div class="chroma-meta-field">
				<label for="location_phone"><?php _e('Phone', 'chroma-early-start'); ?></label>
				<input type="text" id="location_phone" name="location_phone" value="<?php echo esc_attr($phone); ?>"
					placeholder="(770) 555-1234" />
			</div>

			<div class="chroma-meta-field">
				<label for="location_email"><?php _e('Email', 'chroma-early-start'); ?></label>
		...
		<div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px;">
			<div class="chroma-meta-field">
				<label for="location_latitude"><?php _e('Latitude', 'chroma-early-start'); ?></label>
				<input type="text" id="location_latitude" name="location_latitude"
					value="<?php echo esc_attr($latitude); ?>" placeholder="33.9562" />
				<small><?php _e('For map integration (optional)', 'chroma-early-start'); ?></small>
			</div>

			<div class="chroma-meta-field">
				<label for="location_longitude"><?php _e('Longitude', 'chroma-early-start'); ?></label>
	...
	<div class="chroma-meta-section">
		<h4><?php _e('Google Maps & GMB', 'chroma-early-start'); ?></h4>

		<div class="chroma-meta-field">
			<label for="location_gmb_url"><?php _e('Google My Business URL', 'chroma-early-start'); ?></label>
			...
			<small><?php _e('Direct URL to your Google My Business listing. Used by AI to enhance content generation.', 'chroma-early-start'); ?></small>
		</div>

		<div class="chroma-meta-field">
			<label for="location_maps_embed"><?php _e('Google Maps Embed Code', 'chroma-early-start'); ?></label>
			...
			<small><?php _e('Paste the full iframe embed code from Google Maps. Go to Google Maps → Share → Embed a map', 'chroma-early-start'); ?></small>
		</div>
	</div>

	<div class="chroma-meta-section">
		<h4><?php _e('Tour Booking', 'chroma-early-start'); ?></h4>

		<div class="chroma-meta-field">
			<label for="location_tour_booking_link"><?php _e('Tour Booking Link', 'chroma-early-start'); ?></label>
			...
			<small><?php _e('External link for "Book a Tour Now" button (e.g., online scheduling system)', 'chroma-early-start'); ?></small>
		</div>
	</div>

	<div class="chroma-meta-section">
		<h4><?php _e('School Pickups', 'chroma-early-start'); ?></h4>

		<div class="chroma-meta-field">
			<label for="location_school_pickups"><?php _e('Elementary Schools', 'chroma-early-start'); ?></label>
			...
			<small><?php _e('Enter one school name per line. These will be displayed on the location page.', 'chroma-early-start'); ?></small>
		</div>
	</div>

	<div class="chroma-meta-section">
		<h4><?php _e('Location SEO Content', 'chroma-early-start'); ?></h4>

		<div class="chroma-meta-field">
			<label for="location_seo_content_title"><?php _e('SEO Content Title', 'chroma-early-start'); ?></label>
			...
			<small><?php _e('Title for the location-specific content section (e.g., "Early Education and Care in [City], GA")', 'chroma-early-start'); ?></small>
		</div>

		<div class="chroma-meta-field">
			<label for="location_seo_content_text"><?php _e('SEO Content Description', 'chroma-early-start'); ?></label>
			...
			<small><?php _e('Location-specific description highlighting the area, accessibility, and unique features. This content appears at the bottom of the location page.', 'chroma-early-start'); ?></small>
		</div>
	</div>

	<div class="chroma-meta-section">
		<h4><?php _e('Location-Specific FAQs (LLM SEO)', 'chroma-early-start'); ?></h4>

		<div class="chroma-meta-field">
			<label for="location_faq_items"><?php _e('FAQ Items', 'chroma-early-start'); ?></label>
			...
			<small><?php _e('Enter FAQ items in format: Question|Answer (one per line). These appear in JSON endpoints for LLMs.', 'chroma-early-start'); ?></small>
		</div>
	</div>

	<div class="chroma-meta-section">
		<p><strong><?php _e('Note:', 'chroma-early-start'); ?></strong>
			<?php _e('Use the "Featured Image" box in the sidebar to set the hero image for this location. Programs available at this location can be managed from the Programs admin section.', 'chroma-early-start'); ?>
		</p>
	</div>
	<?php
}
 ...
 */
function earlystart_save_location_custom_fields($post_id)
{
	// Verify nonce
	if (!isset($_POST['earlystart_location_meta_nonce_field']) || !wp_verify_nonce(wp_unslash($_POST['earlystart_location_meta_nonce_field']), 'earlystart_location_meta_nonce')) {
		return;
	}
