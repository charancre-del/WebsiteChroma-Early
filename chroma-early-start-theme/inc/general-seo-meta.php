...
function earlystart_register_general_seo_meta_box()
{
    ...
    foreach ($screens as $screen) {
        add_meta_box(
            'chroma-general-seo',
            __('SEO Meta', 'chroma-early-start'),
...
function earlystart_render_general_seo_meta_box($post)
{
    ...
    <div class="chroma-seo-field" style="margin-bottom: 15px;">
        <label for="meta_description" style="display: block; font-weight: 600; margin-bottom: 5px;">
            <?php _e('Meta Description', 'chroma-early-start'); ?>
        </label>
        <textarea id="meta_description" name="meta_description" rows="4" style="width: 100%;"
            placeholder="<?php _e('Enter custom meta description...', 'chroma-early-start'); ?>"><?php echo esc_textarea($meta_description); ?></textarea>
        <p class="description" style="font-size: 12px; margin-top: 5px;">
            <?php _e('Leave empty to use the auto-generated description.', 'chroma-early-start'); ?>
        </p>
    </div>

    <div class="chroma-seo-field">
        <label for="meta_keywords" style="display: block; font-weight: 600; margin-bottom: 5px;">
            <?php _e('Meta Keywords', 'chroma-early-start'); ?>
        </label>
        <textarea id="meta_keywords" name="meta_keywords" rows="2" style="width: 100%;"
            placeholder="<?php _e('keyword1, keyword2, keyword3', 'chroma-early-start'); ?>"><?php echo esc_textarea($meta_keywords); ?></textarea>
        <p class="description" style="font-size: 12px; margin-top: 5px;">
            <?php _e('Comma-separated list. Leave empty to use auto-generated keywords.', 'chroma-early-start'); ?>
        </p>
    </div>
    </div>
    ...
    <div style="margin-top: 15px; border-top: 1px solid #eee; padding-top: 15px;">
        <button type="button" id="chroma-seo-autofill" class="button">
            ...
            <?php _e('Auto-Fill with AI', 'chroma-early-start'); ?>
        </button>
        <span class="spinner" id="chroma-seo-spinner" style="float: none; margin: 0 5px;"></span>
        <p class="description" style="margin-top: 5px;">
            <?php _e('Generates a description and keywords based on page content.', 'chroma-early-start'); ?>
        </p>
    </div>
    ...
    <script>
        jQuery(document).ready(function ($) {
            $('#chroma-seo-autofill').on('click', function (e) {
                ...
                if (!confirm('<?php _e('This will overwrite existing SEO fields with AI-generated content. Continue?', 'chroma-early-start'); ?>')) {
                    return;
                }
