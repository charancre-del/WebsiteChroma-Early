 ...
 */
function earlystart_privacy_page_meta_boxes() {
	add_meta_box(
		'privacy_page_general',
		'Privacy Policy Settings',
		'earlystart_privacy_page_general_callback',
		'page',
		'normal',
		'high'
	);
	...
	add_meta_box(
		...
		'earlystart_privacy_page_sections_callback',
		'page',
		'normal',
		'high'
	);
}
add_action( 'add_meta_boxes', 'earlystart_privacy_page_meta_boxes' );

/**
 * General Settings Meta Box Callback
 */
function earlystart_privacy_page_general_callback( $post ) {
	wp_nonce_field( 'earlystart_privacy_page_general_nonce', 'earlystart_privacy_page_general_nonce' );
 ...
 */
function earlystart_privacy_page_sections_callback( $post ) {
	wp_nonce_field( 'earlystart_privacy_page_sections_nonce', 'earlystart_privacy_page_sections_nonce' );
 ...
 */
function earlystart_save_privacy_page_meta( $post_id ) {
	// Check if our nonces are set
	if ( ! isset( $_POST['earlystart_privacy_page_general_nonce'] ) &&
	     ! isset( $_POST['earlystart_privacy_page_sections_nonce'] ) ) {
		return;
	}

	// Verify nonces
	$general_nonce_valid = isset( $_POST['earlystart_privacy_page_general_nonce'] ) &&
	                       wp_verify_nonce( $_POST['earlystart_privacy_page_general_nonce'], 'earlystart_privacy_page_general_nonce' );
	$sections_nonce_valid = isset( $_POST['earlystart_privacy_page_sections_nonce'] ) &&
	                        wp_verify_nonce( $_POST['earlystart_privacy_page_sections_nonce'], 'earlystart_privacy_page_sections_nonce' );
...
add_action( 'save_post', 'earlystart_save_privacy_page_meta' );

/**
 * Auto-seed Privacy Policy page with default content
 */
function earlystart_seed_privacy_page_defaults( $post_id ) {
	// Only run for pages
...
add_action( 'save_post', 'earlystart_seed_privacy_page_defaults', 5 );
