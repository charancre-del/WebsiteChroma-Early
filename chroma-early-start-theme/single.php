...
$author_title = get_the_author_meta('description') ?: __('Contributor', 'chroma-early-start');
    ...
    <div class="max-w-7xl mx-auto px-4 lg:px-6 h-[82px] flex items-center justify-between">
      ...
      <nav class="hidden md:flex items-center gap-8 text-sm font-semibold text-brand-ink/80">
        ...
        <a href="<?php echo esc_url($stories_url); ?>" class="hover:text-chroma-blue flex items-center gap-2"><i
            class="fa-solid fa-arrow-left"></i> <?php _e('Back to Stories', 'chroma-early-start'); ?></a>
      </nav>
      <?php $locations_url = earlystart_smart_link('locations'); ?>
      <a href="<?php echo esc_url($locations_url); ?>"
        class="hidden sm:inline-flex items-center gap-2 bg-brand-ink text-white text-xs font-semibold tracking-[0.2em] px-6 py-3 rounded-full shadow-soft"><?php _e('Book Tour', 'chroma-early-start'); ?></a>
    </div>
  </header>
    ...
    <?php if ($related_query->have_posts()): ?>
      <section class="bg-white py-20 border-t border-brand-ink/5">
        <div class="max-w-6xl mx-auto px-4 lg:px-6">
          <h3 class="font-serif text-3xl font-bold mb-8 text-center"><?php _e('More from Chroma', 'chroma-early-start'); ?></h3>
