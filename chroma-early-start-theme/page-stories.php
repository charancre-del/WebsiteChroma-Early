...
<main>
  ...
  <section class="py-20 bg-white text-center">
    <div class="max-w-4xl mx-auto px-4">
      <span class="text-chroma-red font-bold tracking-[0.2em] text-xs uppercase mb-3 block"><?php _e('The Blog', 'chroma-early-start'); ?></span>
      <h1 class="font-serif text-5xl md:text-6xl text-brand-ink mb-6"><?php _e('Chroma Stories', 'chroma-early-start'); ?></h1>
      <p class="text-lg text-brand-ink/90"><?php _e('Parenting tips, classroom spotlights, and insights from our educators.', 'chroma-early-start'); ?></p>
        ...
        <a href="<?php echo esc_url(get_permalink($featured_post_id)); ?>" class="block">
          <div class="relative rounded-[3rem] overflow-hidden shadow-soft group cursor-pointer h-[500px]">
            ...
            <div class="absolute bottom-0 left-0 p-8 md:p-12">
              <span
                class="bg-chroma-yellow text-brand-ink text-[10px] font-bold uppercase px-3 py-1 rounded-full mb-4 inline-block"><?php _e('Featured', 'chroma-early-start'); ?></span>
              ...
              <span class="text-white text-xs font-bold uppercase tracking-widest border-b border-white/40 pb-1"><?php _e('Read Story', 'chroma-early-start'); ?></span>
            </div>
          </div>
        </a>
      </section>
  ...
  <section class="pb-24 px-4 lg:px-6 max-w-7xl mx-auto">
        ...
        <div class="flex justify-center gap-4 mt-12">
          <?php if ($paged > 1): ?>
            <a href="<?php echo esc_url(add_query_arg('paged', $paged - 1)); ?>"
              ...
              <?php _e('&larr; Previous', 'chroma-early-start'); ?>
            </a>
          <?php endif; ?>

          <span class="px-6 py-3 text-sm text-brand-ink/60">
            <?php printf(__('Page %s of %s', 'chroma-early-start'), $paged, $posts_query->max_num_pages); ?>
          </span>
          ...
          <?php if ($paged < $posts_query->max_num_pages): ?>
            <a href="<?php echo esc_url(add_query_arg('paged', $paged + 1)); ?>"
              ...
              <?php _e('Next &rarr;', 'chroma-early-start'); ?>
            </a>
          <?php endif; ?>
        </div>
      <?php endif; ?>
    ...
    <?php else: ?>
      <div class="text-center py-16">
        <p class="text-brand-ink/90 text-lg"><?php _e('No stories found. Check back soon!', 'chroma-early-start'); ?></p>
      </div>
    <?php endif; ?>
